...
const CONFIG = {
supabase: {
url: “https://biheagvyxsvcszubwptz.supabase.co”, // From Supabase → Settings → API
anonKey: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJpaGVhZ3Z5eHN2Y3N6dWJ3cHR6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQzNTIxMzQsImV4cCI6MjA4OTkyODEzNH0._iK0eBtjTgcmAwSlRFr3oqf1J4A2RqpBed91DVrHGG4", // From Supabase → Settings → API
},
originstamp: {
apiKey: “YOUR_ORIGINSTAMP_API_KEY”, // From originstamp.com → API
},
payfast: {
merchantId: “YOUR_PAYFAST_MERCHANT_ID”, // From payfast.co.za → Settings
merchantKey: “YOUR_PAYFAST_MERCHANT_KEY”, // From payfast.co.za → Settings
...
method, headers: { …headers, Prefer: method === “POST” ? “return=representation” : “” },
...
// If API unavailable, still return hash (local proof)
...
// ============================================================
const C = {
gold: “#F5A623”, green: “#00C48C”, dark: “#0A0A0F”, mid: “#13131F”,
card: “#1A1A2E”, border: “rgba(245,166,35,0.15)”, muted: “rgba(255,255,255,0.45)”,
};
...
<button onClick={onClose} style={{ background:“none”, border:“none”, color:C.muted, cursor:“pointer”, marginLeft:“auto”, fontSize:16 }}>×</button>
</div>
);
}
...
```
 ...
 {[[" ","Submit Project","Founders list their idea, funding goal, and equity offer. Takes 10 minutes."],[" ","IP Timestamped","Your idea is hashed on OriginStamp's blockchain — immutable proof of origin."],[" ","Investors Sign NDA","Every investor signs a legally binding NDA before seeing your full details."],[" ","Get Funded","Goal reached → PayFast releases funds. Equity certificates issued automatically."]].map(([icon,title,desc],i)=>(
...
return (
<div style={{ background:C.card, border:“1px solid rgba(255,255,255,0.07)”, borderRadius:20, padding:26, display:“flex”, flexDirection:“column”, gap:14, cursor:“pointer”, transition:“all .2s” }}
...
{daysLeft !== null && daysLeft <= 7 && <div style={{ background:“rgba(239,68,68,0.1)”, border:“1px solid rgba(239,68,68,0.3)”, color:”#ef4444”, padding:“4px 10px”, borderRadius:100, fontSize:10, fontFamily:“Syne”, animation:“pulse 2s infinite” }}> {daysLeft}d left</div>}
</div>
<div>
...
// ============================================================
// FOUNDER FORM — Saves to Supabase + OriginStamp
// ============================================================
function FounderForm({ setPage, addToast }) {
const [step, setStep] = useState(1);
const [form, setForm] = useState({ full_name:””, email:””, phone:””, sa_id:””, business_name:””, cipc_number:””, project_title:””, category:””, description:””, funding_goal:””, equity_offered:””, use_of_funds:””, agreed:false });
...
setLoading(true);
try {
// 1. Timestamp the IP on OriginStamp
...
```
 ...
 const result = await supabase.from("founders").insert(payload);
...
Your project <strong style={{ color:”#fff” }}>”{submitted.project}”</strong> has been received and your IP is {submitted.ipSuccess ? “blockchain-timestamped on OriginStamp” : “locally hashed”}. We’ll review and go live within 48 hours.
</p>
...
```
 ...
 <div><label style={lbl}>Project Description * <span style={{ color:C.muted }}>(min 100 words)</span></label>
 ...
 <div><label style={lbl}>Use of Funds *</label>
 ...
 {step>1 ? <button onClick={()=>setStep(s=>s-1)} style={{ background:"rgba(255,255,255,0.05)", color:"rgba(255,255,255,0.65)", border:"1px solid rgba(255,255,255,0.1)", padding:"11px 24px", borderRadius:100, fontFamily:"Syne", cursor:"pointer" }}>← Back</button> : <div/>}
 {step<3
 ? <button onClick={()=>setStep(s=>s+1)} disabled={step===1&&(!form.full_name||!form.email||!form.sa_id)||step===2&&(!form.project_title||!form.category||!form.description)}
 style={{ background:C.gold, color:"#000", border:"none", padding:"11px 28px", borderRadius:100, fontFamily:"Syne", fontWeight:700, cursor:"pointer" }}>Continue →</button>
 : <button onClick={handleSubmit} disabled={!form.agreed||!form.funding_goal||!form.equity_offered||loading}
...
// ============================================================
// INVESTOR FORM — Saves to Supabase, NDA, then PayFast
// ============================================================
function InvestForm({ setPage, projects, addToast }) {
const [step, setStep] = useState(1);
const [form, setForm] = useState({ full_name:””, email:””, phone:””, sa_id:””, investment_range:””, risk_acknowledged:false, nda_agreed:false, selectedProject:””, amount:”” });
const [loading, setLoading] = useState(false);
...
signature_hash: await (async()=>{ const d=new TextEncoder().encode(investorId+Date.now()); const h=await crypto.subtle.digest(“SHA-256”,d); return Array.from(new Uint8Array(h)).map(b=>b.toString(16).padStart(2,“0”)).join(””); })(),
});
// Update investor
if (investorId) await supabase.from(“investors”).update({ nda_signed:true, nda_signed_at:new Date().toISOString() }, `id=eq.${investorId}`);
setStep(3);
...
addToast(“Please select a project and enter at least R50”, “error”); return;
}
...
```
 ...
 <div><label style={lbl}>Investment Range</label>
 ...
 By signing, you ("<strong>Investor</strong>") agree all project details, business plans, financials, and intellectual property shared through YouthVest are strictly confidential. You agree not to disclose, reproduce, or use this information for any purpose other than evaluating an investment. Breach may result in legal action under South African law and permanent platform removal. Governed by the laws of the Republic of South Africa.
 </div>
 <label style={{ display:"flex", gap:12, cursor:"pointer" }}>
 ...
 <div><label style={lbl}>Select Project *</label>
 ...
 <div><label style={lbl}>Investment Amount (R) * <span style={{ color:C.muted }}>— minimum R50</span></label>
 ...
 You'll be redirected to PayFast's secure payment page. After payment, your equity certificate will be issued within 24 hours.
 </div>
 </div>
 )}
 ...
 <div style={{ display:"flex", justifyContent:"space-between", marginTop:24, gap:12 }}>
 {step>1 ? <button onClick={()=>setStep(s=>s-1)} style={{ background:"rgba(255,255,255,0.05)", color:"rgba(255,255,255,0.65)", border:"1px solid rgba(255,255,255,0.1)", padding:"11px 24px", borderRadius:100, fontFamily:"Syne", cursor:"pointer" }}>← Back</button> : <div/>}
 {step===1 && <button onClick={handleRegister} disabled={!form.full_name||!form.email||!form.sa_id||!form.risk_acknowledged||loading} style={{ background:form.full_name&&form.email&&form.sa_id&&form.risk_acknowledged?C.gold:"rgba(255,255,255,0.08)", color:form.full_name&&form.email?"#000":"rgba(255,255,255,0.3)", border:"none", padding:"11px 28px", borderRadius:100, fontFamily:"Syne", fontWeight:700, cursor:"pointer", display:"flex", alignItems:"center", gap:10 }}>
 {loading?<><Spinner/> Saving...</>:"Continue →"}
 </button>}
 {step===2 && <button onClick={handleNDA} disabled={!form.nda_agreed||loading} style={{ background:form.nda_agreed?C.gold:"rgba(255,255,255,0.08)", color:form.nda_agreed?"#000":"rgba(255,255,255,0.3)", border:"none", padding:"11px 28px", borderRadius:100, fontFamily:"Syne", fontWeight:700, cursor:form.nda_agreed?"pointer":"not-allowed", display:"flex", alignItems:"center", gap:10 }}>
 {loading?<><Spinner/> Signing...</>:"Sign NDA & Continue "}
 </button>}
 {step===3 && <button onClick={handleInvest} disabled={!form.selectedProject||!form.amount||Number(form.amount)<50} style={{ background:form.selectedProject&&form.amount?C.gold:"rgba(255,255,255,0.08)", color:form.selectedProject&&form.amount?"#000":"rgba(255,255,255,0.3)", border:"none", padding:"11px 28px", borderRadius:100, fontFamily:"Syne", fontWeight:700, cursor:"pointer" }}>
 Pay via PayFast →
 </button>}
 </div>
 </div>
...
// ============================================================
function Dashboard({ projects, investments, addToast }) {
...
```
 ...
 {[["Total Raised","R"+totalRaised.toLocaleString("en-ZA"),C.gold],["Live Projects",liveProjects,C.green],["Total Investors",investments.length,"#6366f1"],["Pending Review",projects.filter(p=>p.status==="pending").length,C.gold]].map(([l,v,color])=>(
 ...
 {projects.length===0 ? <p style={{ color:C.muted, fontSize:13 }}>No projects yet. Share your listing link!</p> :
 ...
 {projects.length===0 ? <p style={{ color:C.muted }}>No projects submitted yet.</p> :
 projects.map(p=>(
 ...
 {tab==="investments" && (
 <div style={{ animation:"fadeUp .3s ease" }}>
 {investments.length===0 ? <p style={{ color:C.muted }}>No investments yet.</p> :
...
// App still works — just shows empty state
} finally { setLoading(false); }
};
load();
}, []);
